  ...
  let kTitleMinimumScaleFactor: CGFloat
    ...
    fileprivate func setup() {
      ...
      view.autoresizingMask = [UIView.AutoresizingMask.flexibleHeight, UIView.AutoresizingMask.flexibleWidth]
        ...
        // Colours
        contentView.backgroundColor = UIColor(rgb: 0xE4EFD4)
        viewText.backgroundColor = UIColor(rgb: 0xE4EFD4)
        labelTitle.textColor = UIColor(rgb: 0x80B239)
...
//        let titleOffset : CGFloat = appearance.showCircularIcon ? 0.0 : -12.0
        ...
        case .vertical:
            for btn in buttons {
                btn.frame = CGRect(x:12, y:y, width:appearance.kWindowWidth - 24, height:46)
              btn.layer.cornerRadius = 23
                y += appearance.kButtonHeight
            }
        case .horizontal:
            for btn in buttons {
                btn.frame = CGRect(x:buttonX, y:y, width: widthEachButton, height:46)
              btn.layer.cornerRadius = 23
